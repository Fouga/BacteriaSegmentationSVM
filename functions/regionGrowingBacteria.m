function RED_mask = regionGrowingBacteria(RED_mask, red_8, gr_8, bl_8,options)


% convert the image to 8bit. The object should be visible for the growing.
% for bacteria segmentation the red channel is not needed. Therefore, we
% use only green and blue for improving the segmentation.
if options.IncludeRed ==1
    r=red_8;
    r(r> options.redThresh)= options.redThresh;
...
m=gr_8;
m(m> options.greenThresh)= options.greenThresh;
im2_8 = uint8(double(m)./double(max(m(:)))*2^8);

n = bl_8;
n(n> options.blueThresh)= options.blueThresh;
...
%         if size (IM,1)<rect2(4)+1
    ...
    elseif (isempty(find(y==seeds(i,1) & x==seeds(i,2))) || isempty(x)) && options.IncludeRed==1
        seed_point = seeds(i,:);
        dist_inten = 20;
        IM = rgb2gray(rgbIm);
        [~,J1] = regionGrowing(IM,[seed_point(2) seed_point(1)],dist_inten); 
